...
void PD_Write(FT_EEPROM_PD *pd)
{
    ...
    PD_CFG_EnablePassThrough(pd, TRUE, GPIO4, GPIO3, GPIO0); // GPIO4=CHARGE_THROUGH , GPIO3 = FULL_POWER GPIO0 = VSAFE5V
    PD2_LoadEnable(pd, NOT_APPLICABLE);
    ...
    /* Note: When Pass Through is enabled, there is no need to configure the Source PDO and Sink2 PDO */    
}
