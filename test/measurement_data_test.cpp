...
namespace test {
    ...
    public:
        ...
        TEST_METHOD(test_construction) {
            {
                auto m = measurement_data(42, 2.0f, 3.0f, 6.1f);
            ...
            {
                auto m = measurement_data(42, 2.0f, 3.0f);
                Assert::AreEqual(2.0f, m.voltage(), L"voltage", LINE_INFO());
                Assert::AreEqual(3.0f, m.current(), L"current", LINE_INFO());
            ...
            {
                auto m = measurement_data(42, 2.0f);
            ...
            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement_data(42, measurement_data::invalid_value, 3.0f);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement_data(42, 2.0f, measurement_data::invalid_value);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement_data(42, measurement_data::invalid_value);
            });
        }
