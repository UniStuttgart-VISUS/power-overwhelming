...
public:
    ...
    TEST_METHOD(test_nvapi_descriptions) {
        typedef detail::nvapi_sensor type;
...
#if defined(POWER_OVERWHELMING_WITH_NVAPI)
    TEST_METHOD(test_nvapi_sensor_creation) {
        typedef detail::nvapi_sensor type;
        ...
        dummy.configuration = std::make_unique<detail::sensor_array_configuration_impl>();
        dummy.configuration->interval = std::chrono::milliseconds(100);
...
#if defined(POWER_OVERWHELMING_WITH_NVML)
    TEST_METHOD(test_nvml_sensor_creation) {
        typedef detail::nvml_sensor type;
