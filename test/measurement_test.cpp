...
namespace test {
    ...
    public:
        ...
        TEST_METHOD(test_construction) {
            {
                auto m = measurement(L"test", 42, 2.0f, 3.0f, 6.1f);
            ...
            {
                auto m = measurement(L"test", 42, 2.0f, 3.0f);
                Assert::AreEqual(2.0f, m.voltage(), L"voltage", LINE_INFO());
                Assert::AreEqual(3.0f, m.current(), L"current", LINE_INFO());
            ...
            {
                auto m = measurement(L"test", 42, 2.0f);
            ...
            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(nullptr, 42, 2.0f, 3.0f, 6.1f);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(nullptr, 42, 2.0f, 3.0f);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(nullptr, 42, 2.0f);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(L"test", 42, measurement::invalid_value, 3.0f);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(L"test", 42, 2.0f, measurement::invalid_value);
            });

            Assert::ExpectException<std::invalid_argument>([](void) {
                auto m = measurement(L"test", 42, measurement::invalid_value);
            });
        }
        ...
        TEST_METHOD(test_copy) {
            {
                ...
                Assert::AreEqual(m.timestamp(), n.timestamp(), L"timestamp", LINE_INFO());
