...
1. Sort the range [`begin`, `end`[ such that all `sensor_description`s for the sensor type are at the begin of the range. You can use any method you want to achieve this, but the recommended way is using the `move_front_if` function from `sensor_utilities.h`.
